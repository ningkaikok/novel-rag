...
- **数据库**：PostgreSQL + pgvector；小说兼容链路默认继续读写 V1 `novel_chunks`，通用文档已写入 `knowledge_v2`，并可通过显式开关或灰度模式接入真实问答检索。
...
- **前端**：React + Vite + TypeScript + Ant Design（`frontend/`），书卷气界面「书虫」。左侧目录使用通用知识库/文档术语；通用文档支持上传、版本历史、删除、重建和失败恢复，旧小说入口继续兼容。前后端类型契约由 OpenAPI 生成（`schemas.py` → `openapi.json` → `api-generated.ts`，CI 有 drift 检查），改 Pydantic 模型后跑 `uv run python scripts/export_openapi.py && cd frontend && npm run gen:api`。
...
**终端 2 — 前端 dev server（端口 45173，刻意避开 Vite 默认的 5173——本机常有别的
项目占用这个端口）：**
...
> 访问时，请自己补上防火墙或反向代理鉴权。
>
> **MCP（实验性）**：`uv run python scripts/mcp_server.py` 提供通用知识库只读 MCP 服务器
> （stdio），可在 Claude Code 等客户端注册后查询目录和文档证据；小说通过 legacy adapter
> 兼容。Agent/MCP 当前不读取 V2 数据库、不提供认证或多租户权限，引用摘录最多 80 字。
>
...
- 文档上传支持 `.txt`、`.md`、`.markdown` 和文本型 `.pdf`；暂不支持 EPUB，也不对扫描版 PDF 做 OCR。单文件默认上限为 20 MiB，并限制 PDF 页数和解析后的片段数。
...
- 网页上传没有认证、用户隔离、用户级存储配额或内容审核，仍适合本地单人使用。文件大小和格式限制已实现；对外提供服务前仍需补齐身份权限、配额和运维保护。
