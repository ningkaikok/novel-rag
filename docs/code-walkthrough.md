...
M3.3 之后，`src/index_quality.py` 是入库前的质量门禁：用真实 tokenizer 统计最终
embedding 输入的 token 分布，空输入、静默截断、异常向量或维度不一致会硬性阻止当前书
替换旧索引；报告摘要随 `index_manifest.quality_report` 落库。想在不写库的情况下
先看报告，跑 `python scripts/check_index_quality.py --novel <文件>`。
...
M3.3.5 之后 rag.py 已按职责拆分：书名识别与意图判断在 `novel_match.py`，
`SourceChunk` 与 trace 候选在 `chunk_model.py`，向量/BM25/结构性召回方法在
`retrieval_mixins.py`，prompt 与生成在 `generation_mixin.py`；rag.py 本体（约 530 行）
只保留编排层——混合检索流水线、层级导航和全文直通的调度逻辑。对外 import 路径
不变，下面的阅读顺序仍然成立：
