...
不额外引入 Redis/Celery。任务卡状态以进程内存为运行时事实来源，同时快照落库到
`index_task_runs` 表（M3.3.5）：重启后侧栏回退读库恢复上一次任务的最后已知状态，
遗留的 active 任务被启动逻辑如实标记为 failed 并提示重试。真正需要跨重启保存的
数据边界仍是 `index_manifest` 和单书事务——落库只是"最后已知状态"的副本，
数据一致性不依赖它。
