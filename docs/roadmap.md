...
- [ ] **工具输出的 token 预算闸**（与会话历史压缩是两件事，但同属上下文预算）：Agent Lab 的
      `read_neighbors` / `get_chapter` 可以返回整章，大部头的一章本身就可能撑爆 prompt。
      应设两道闸——**在读取任何内容之前**先限制单次请求的定位符数量，读到内容之后再限制字符/token
      上限，并把截断如实写进 trace。目录类输出同理：超长目录按层级摘要，而不是整份塞进 prompt。
      当前只有 3～5 步的步数上限，没有单步输出体积上限
...
- [ ] 增加 token 预算与可观测字段，记录历史长度、改写耗时、摘要耗时和截断原因
      （2026-09-05：历史长度、带入轮数与截断原因已进 trace；摘要耗时待滚动摘要落地）
