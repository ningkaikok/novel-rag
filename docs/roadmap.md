...
>
> 2026-09-23 执行统计时，本机 PostgreSQL `127.0.0.1:5432` 未监听，暂无真实 `parse_mode`
> 样本可汇总；失败率和排期结论仍待数据库恢复后补跑，不能据此提前改协议。
...
- [ ] 在固定大部头语料上评测整章扩展的召回与延迟变化，并记录逐例回退情况。
  （2026-09-23：当前工作树仅含两篇约 3.4 KB 原创短篇；固定大部头评测语料不在仓库，
  本机 PostgreSQL 未启动，且 bge-small-zh 模型缓存缺失、Hugging Face 连接被拒，无法重跑。）
...
- [ ] 在更大、更多样的语料上复核关系抽取质量；达到验收门槛前保持 `GRAPH_ENABLED=0`。
  （2026-09-23：当前仓库只有两篇原创短篇可供关系评测，合计 6,793 字节；缺少更大且
  多样的授权评测语料，现有 16 条基线不能替代该复核。）
...
- [x] M6.2：建立本地只读 Tool Gateway 基础，集中做权限与参数校验、出站白名单、Prompt Injection
  ...
  出站工具适配、写操作审批/重试和跨进程共享熔断仍待真实需求出现后补齐。
