...
- **对「全文模式」不适用**：书小到能整本塞进模型窗口时（见
  `config.FULL_TEXT_MAX_CHARS`），检索被跳过、全部片段按**原文顺序**送给模型。
  这时"命中排在第几"没有意义——所有片段都进了 prompt。实测《雾隐山庄》
  就因此从 #1 变成 #2（正确内容在第 2 段），指标看起来退步，实际没有任何损失。
  这是**指标不适用于该模式**，不是检索变差了。
